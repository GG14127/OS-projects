...
void update_need(int ** need, int ** maximum, int ** allocation);//Update the need matrix
//int check_initial_safe();//could be in main
...
//void destroy();//free  could be in main
...
//read in inst
	...
	while(TRUE) 
	{
		...
		{
			if (request_resources(arg[0], arg + 1))	//unsafe
...
void initialize(int argc, char *argv[]) {
	...
	static int data;
	while(~fscanf(fp, "%d", &data)) //while(fscanf(fp, "%d", &data)!=EOF)
	{
		//double the array if full
		if (customer_num == capacity) 
		{
			int ** tmp;
			tmpm = (int **) malloc (sizeof(int *) * capacity * 2);
